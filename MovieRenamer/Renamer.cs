...
namespace MovieRenamer
{
    ...
    {       
        ...
        static async Task<Movie> GetMovie(int id,string language)
        {
            var client = new ServiceClient("dc272f1ee966ae5074280085a087c231");
            return await client.Movies.GetAsync(id, language, true, System.Threading.CancellationToken.None);
        }

        static async Task<Movies> SearchForMovie(string fileName, string language)
        {
            ...
            Console.WriteLine("Searching for film '{0}' language '{1}'", fileName, language);
            var client = new ServiceClient("dc272f1ee966ae5074280085a087c231");
            return await client.Movies.SearchAsync(fileName, language, true, 1, System.Threading.CancellationToken.None);
        }
    }
}
